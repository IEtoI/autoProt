...
def corrMap(df, columns, cluster=False, annot=None, cmap="YlGn", figsize=(7,7),
            saveDir = None, saveType="pdf", saveName="pairPlot"):
    corr = df[columns].corr()
    if cluster == False:
        plt.figure(figsize=figsize)
        sns.heatmap(corr, cmap=cmap, square=True, cbar=False, annot=annot)
    else:
        sns.clustermap(corr, cmap=cmap, annot=annot)
...
def intensityRank(data, rankCol="log10_Intensity" ,label=None, n=5, title="Rank Plot", figsize=(15,7), file=None,
    ...
    if label is not None:
        top_y = y.iloc[-1]
        top_yy = np.linspace(top_y-n*0.2, top_y, n)
        top_oy = y[-n:]
        top_xx = x[-n:]
        ...
        low_y = y.iloc[0]
        low_yy = np.linspace(low_y, low_y+n*0.2, n)
        low_oy = y[:n]
        low_xx = x[:n]
...
def vennDiagram(df, figsize=(10,10), retFig=False):
...
def logIntPlot(df, logFC, Int, fct=None, annot=False, interactive=False,
    ...
    if interactive == False:
        ...
        plt.figure(figsize=figsize)
        ax=plt.subplot()
        plt.scatter(df[logFC].loc[unsig], df[Int].loc[unsig], color=bg_col,alpha=.33, s=2, label="background")
...
def MAPlot(df, x, y, interactive=False, fct=None,
    sig_col="green", bg_col="lightgray", title="Volcano Plot", figsize=(6,6), hover_name=None):
    """
    not implemented atm only dummy code below!!!!
    """
    df = df.copy(deep=True)
    df["M"] = df[x] - df[y]
    df["A"] = 1/2 * (df[x]+df[y])
